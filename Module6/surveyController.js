...
module.exports = function(app){
    ...
    app.get('/analysis', function(req, res){
        var first = readData("first");
        var last = readData("last");
        var email = readData("email");
        var phone = readData("phone");
        var stores = readData("stores");
        var rate = readData("rate");
        var recommend = readData("recommend");
        var chatbox = readData("chatbox");
        var comments = readData("comments")
        res.render('showResults', {results: [first,last,email,phone,stores,rate,recommend,chatbox,comments]});
        console.log([first,last,email,phone,stores,rate,recommend,chatbox,comments]);
    });
    ...
    app.post('/niceSurvey', urlencodedParser, function(req, res){
        ...
        for (var key in json){
            ...
            if ((key == "stores") && (json[key].length === 2)){
